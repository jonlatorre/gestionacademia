...
class GestionacademiaCtrl (Controller):
    ...
    def register_view(self, view):
        ...
        tabla_alumnos=[dict(nombre='Número',dato=0),dict(nombre='Apellido1',dato=2),
            dict(nombre='Apellido2',dato=3),dict(nombre='Nombre',dato=4),dict(nombre='Grupo',dato=7),
            dict(nombre='Telefono1',dato=8),dict(nombre='Clases',dato=9)]
        ...
        tabla_cursos=[dict(nombre='Nombre',dato=1),dict(nombre='Examen',dato=2),
            dict(nombre='Nivel',dato=3),dict(nombre='Precio',dato=4),dict(nombre='Nota Aprobado',dato=5)]
...
##        self.model.festivo.cargar(id)
    ...
    def on_vertodosalumnos_activate(self, widget):
        debug("Cambiamos el modelo para ver todos los alumnos")
        self.model.alumno.ver_todos = True
        self.model.alumno.rellenar_lista()

    def on_veralumnosactivos_activate(self, widget):
        debug("Cambiamos el modelo para ver solo los alumnos activos")
        self.model.alumno.ver_todos = False
        self.model.alumno.rellenar_lista()
    ...
    def on_tv_calendario_row_activated(self, widget, *args):
        ...
        self.model.festivo.cargar(id)
        self.lanzar_festivo(id)
    ##Para los filtros del TV de alumnos
    def alum_filtro_changed(self,widget,parm1=None,param2=None,param3=None):
        texto = widget.get_text()
        #print texto,parm1,param2,param3
        debug("Ha cambiado el filtro")
        if len(texto)>2 or len(texto)==0:
            debug("Ya son tres carácteres (o está limpio) y podemos buscar")
            self.model.alumno.rellenar_lista()
    ...
    def cargar_nota(self):
        ...
        try:
            ...
            self.log.debug("Cargar nota: tenemos el mes %s, grupo %s y el alumno %s"%(trimestre,grupo,alumno))
    ...
    def imp_notas_grupo(self,widget):
        trimestre = cb_get_active(self.view['cb_trimestre'])
        grupo = cb_get_active(self.view['cb_notas_grupos'])
        self.log.debug("Intentando imprimir las notas del grupo %s del trimeste %s"%(grupo,trimestre))
    ...
    def on_cb_faltas_grupo_changed(self,widget):
        try:
            ...
            ##print "Error al cambiar al combo"
            return -1
        self.model.grupo.cargar(id)
        self.cargar_falta()
    ...
    def cargar_falta(self):
        print "Intentamos cargar una falta, pero primero limpiamos"
    ...
    def focus_faltas(self,widget,args):
        ##print "hooola"
        self.log.debug("Desbloqueamos el check de la parte de faltas")
    ...
    def on_factura_generar(self,widget):
        ...
        if self.view['facturar_medio_mes'].get_active():
            self.log.debug("Vamos a facturar solo medio mes")
            medio=True
        else:
            self.log.debug("Cobramos el mes completo")
            medio = False
        lista = []
        lista_alumnos=[]
        
        ##Si tenemos que facturar a grupos sueltos miramos el treeview
        if self.view['facturar_grupos_sueltos'].get_active():
            self.log.debug( "Vamos a facturar grupos sueltos")
            ...
            for path in pathlist:
                ...
                lista.append(id)
                grupo = Grupo.get(id)
                for asistencia in grupo.alumnos:
                    lista_alumnos.append(asistencia.alumnoID)
            self.log.debug("Elegidos los grupos")
            self.log.debug(lista)
        if self.on_comprobar_cuentas_alumnos(widget,lista_alumnos) == False:
            return -1
        respuesta, error = self.model.facturar(lista,medio)
        if error:
            texto = "Hemos tenido problemas al generar los cargos bancario con los siguientes alumnos: "
            for alumno in respuesta:
                texto += ", %s"%alumno
            texto += "NO hemos generado la facturación. Por favor compruebe que todos los datos del alumno(s) son correctos (CCC, precio, etc)"
            debug(texto)
            mostrar_aviso(texto,"Problema generando los cargos bancarios")
        else:
            mostrar_aviso(respuesta,"Facturacion")
    ...
    def on_tv_profesores_button_press_event(self, treeview, event):
        ##print "Boton en profesores"
        pass
    def on_tv_alumnos_button_press_event(self, treeview, event):
##        print "Intentamos mostrar el pop-up"
        if event.button == 3:
            x = int(event.x)
            y = int(event.y)
            time = event.time
    ...
    def on_calendario_month_changed(self,cal):
        cal.clear_marks()
        self.model.festivo.marcar_dias(cal)
    def on_comprobar_cuentas_alumnos(self,widget,lista=[]):
        pendientes = self.model.alumno.comprobarCuentas(lista)
        if len(pendientes) != 0:
            texto = "Los siguienetes alumnos no tienen una cuenta bancaria correcta. Compruebe si deben pagar en metálico o hay algún error \n\n\n"
            for cont, elemento in enumerate(pendientes):
                texto = texto + "%s: %s"%(elemento[0],elemento[1])
                if cont % 2 == 0 and cont != 0:
                    texto = texto + "\n"
                else:
                    texto = texto + " | \t"

            mostrar_aviso(texto,"Datos bancarios erroneos")
            return False
        else:
            return True
...
Este proceso puede durar unos minutos. Por favor espere a que desaparezca esta ventana y no cierre la aplicación
    ...
    def register_adapters(self):
        ...
        #Para alumnos
        self.adapt("alumno.alum_filtro_nomb","alum_filtro_nomb")
        self.adapt("alumno.alum_filtro_ape1","alum_filtro_ape1")
        self.adapt("alumno.alum_filtro_ape2","alum_filtro_ape2")
