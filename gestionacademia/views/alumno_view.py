...
class AlumnoView(View):
    ...
    """AlumnoView handles only the graphical representation of the
    ...
    def __init__(self, parent=None, stand_alone=True):
        ...
        ui = os.path.join(_config.get_data_path(), 'ui', 'AlumnoDialog.ui')
        if not os.path.exists(ui):
            ui = None
        
        # Top window/widget check.
        if stand_alone: top_widget = 'alumno_dialog'
    ...
    def run(self):
        ...
        """Display the AlumnoDialog window."""
        
        res = self['alumno_dialog'].run()
        #self['alumno_dialog'].destroy()
        return res
