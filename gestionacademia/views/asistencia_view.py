...
class AsistenciaView(View):
    ...
    def __init__(self, parent=None, stand_alone=True):
        ...
        ui = os.path.join(_config.get_data_path(), 'ui', 'AsistenciaDialog.ui')
        if not os.path.exists(ui):
            ui = None
        
        # Top window/widget check.
        if stand_alone: top_widget = 'asistencia_dialog'
    ...
    def run(self):
        ...
        """Display the AsistenciaDialog window."""
        
##        res = self['asistencia_dialog'].run()
        self['asistencia_dialog'].run()
        self['asistencia_dialog'].destroy()
##        return res
