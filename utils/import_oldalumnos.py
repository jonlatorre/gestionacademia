...
##    Asistencia.createTable()
    ...
    while len(cadena):
        ...
        if id > 72 and (marca == 'A' or marca == 'M'):
            ...
            print "Recupetrando alumno de baja %s"%id
...
##            print int(descuento)
            ...
            try:
                fecha_nac = convertir_fecha(fecha_nac)
            except:
                fecha_nac = convertir_fecha('19800101')
            try:
                a=Alumno(id=id,nombre=nombre,apellido1=apellido1,apellido2=apellido2,
                    telefono1=str(telefono),fecha_nacimiento=fecha_nac,banco=ban,
                    ciudad=poblacion,direccion=direccion,sucursal=sucursal,cuenta=cuenta,dc=dc,
                    dni=str(dni),activo=False)
            except:
                print "Fallo recuperando %s"%id
